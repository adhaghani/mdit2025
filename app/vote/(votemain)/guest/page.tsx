...
const GuestVotingPage = () => {
  ...
  const onGuestLoginSubmit = async (data: z.infer<typeof guestFormSchema>) => {
    ...
    // Check if this specific email has been used
    if (guestEmail && guestEmail === data.email) {
      toast.error("This email has already been used to vote.");
      return;
    }
    ...
    try {
      const response = await fetch("/api/vote/guest/data", {
        method: "GET",
        headers: {
      ...
      if (result.success) {
        toast.success("Welcome! You can now vote as a guest.");
        setStep("pitching");
        // Store guest email for duplicate prevention
        localStorage.setItem("guest-email", data.email);
      } else {
        toast.error(result.error || "Failed to load voting data");
      }
    } catch (error) {
  ...
  const handleVoteSubmission = async () => {
    ...
    try {
      ...
      if (!guestEmail) {
        toast.error("Guest email not found. Please start over.");
        setStep("login");
        return;
      }

      const response = await fetch("/api/vote/guest/submit", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        ...
        <Card className="w-full max-w-md mx-auto">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl font-bold">
              MDIT 2025 Guest Voting
            </CardTitle>
            <Text as="p" className="text-muted-foreground">
              Enter your email to vote as a guest
            </Text>
          </CardHeader>
          <CardContent>
            <Form {...guestForm}>
              <form
                onSubmit={guestForm.handleSubmit(onGuestLoginSubmit)}
                className="space-y-6"
              >
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        Enter your email address to vote as a guest
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
          ...
          <CardContent>
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-2">
                ...
                >
                  Guest Voting Guidelines
                </Text>
                <ul className="text-xs text-blue-700 dark:text-blue-300 space-y-1">
                  <li>• Guests can vote for any participant or team</li>
                  <li>• Each email address can only be used once</li>
                  <li>• All three categories must be completed to submit</li>
                  <li>
                    • Your vote will be saved locally to prevent duplicate
                    voting
                  </li>
                </ul>
              </div>
  ...
  const renderPitchingStep = () => {
    ...
    const { participants } = votingData.data;
    ...
    <BlurFade delay={0.25} className="w-full max-w-md mx-auto">
      <Card className="w-full max-w-4xl mx-auto">
        <CardHeader className="text-center">
          ...
          <Text as="p" className="text-muted-foreground">
            Thank you for participating in MDIT 2025 guest voting. Your votes
...
export default GuestVotingPage;
